...
type ChannelType = Omit<PrismaChannelType, 'createdAt' | 'updatedAt'> & {
  createdAt: string
  updatedAt: string
}
...
export function ChannelTypeForm({ initialData }: ChannelTypeFormProps) {
  ...
  async function onSubmit(data: FormValues) {
    try {
      ...
      const response = await fetch(
        initialData 
          ? `/api/categories/channel-types/${initialData.id}`
          : '/api/categories/channel-types',
        {
      ...
      toast({
        title: '保存成功',
        description: '通道类型信息已更新'
      })
      
      router.push('/admin/categories/channel-types')
      router.refresh()
    } catch (error) {
      toast({
        variant: 'destructive',
