...
export default async function ReviewsPage({
  ...
  const { reviews, total, totalPages } = await getReviews(pageNumber)
    ...
    <div className="container mx-auto py-6 space-y-6">
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                      ...
                      <span>•</span>
                      <span>{review.product.productType?.name || '综合测评'}</span>
                    </div>
      ...
      {totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          <Link
            href={`/reviews?page=${Math.max(1, pageNumber - 1)}`}
            className={cn(
              "px-4 py-2 rounded-md text-sm",
              pageNumber === 1 ? "pointer-events-none opacity-50" : "hover:bg-muted"
            )}
          >
            上一页
          </Link>
          ...
          <Link
            href={`/reviews?page=${Math.min(totalPages, pageNumber + 1)}`}
            className={cn(
              "px-4 py-2 rounded-md text-sm",
              pageNumber === totalPages ? "pointer-events-none opacity-50" : "hover:bg-muted"
            )}
          >
            下一页
          </Link>
