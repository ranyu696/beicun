  ...
  try {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        mainImage: body.mainImage || DEFAULT_IMAGE,  // 使用默认图片
        salesImage: body.salesImage || DEFAULT_IMAGE, // 使用默认图片
        videoUrl: body.videoUrl,
        detailImages: body.detailImages || [],       // 使用空数组
        // 处理产品图片，如果没有则不创建
        ProductImage: body.productImages.length > 0 ? {
          createMany: {
